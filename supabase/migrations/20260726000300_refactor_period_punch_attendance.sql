/*
  Refactor biometric attendance to exact routine-period matching.

  The timestamp is compared exactly as stored in UTC. For example, a punch at
  2026-07-26 02:30:00+00 matches only a routine slot containing 02:30:00.
  It no longer marks every generated session on that date as present.
*/
...
BEGIN
  ...
  /*
    Re-open logs in the selected month so records produced by the previous
    day-wide algorithm are recalculated using exact period times.
  */
  ...
  /*
    Reset only records previously created by biometric synchronization.
    Manually entered or finalized attendance is preserved.
  */
  ...
  /*
    Create default absences only after a routine period has ended.
    Future periods are left without a record until their time passes.
  */
  ...
  /*
    A punch matches exactly one generated session when its stored UTC time is
    inside that session's routine interval.
  */
  WITH matched_punches AS (
    ...
    JOIN public.routine_slots AS slot
     ...
     AND slot.period_number = session.period_number
     AND slot.day_of_week = EXTRACT(DOW FROM session.date)::integer
     AND (log.punched_at AT TIME ZONE 'UTC')::time >= slot.start_time
     AND (log.punched_at AT TIME ZONE 'UTC')::time < slot.end_time
    WHERE (log.punched_at AT TIME ZONE 'UTC')::date
      BETWEEN p_month_start AND v_month_end
  ...
  /*
    A device log links to the one attendance record for its matched period.
    Logs outside routine intervals remain unprocessed and visible for review.
  */
  UPDATE public.device_logs AS log
  SET
    processed = true,
...
COMMENT ON FUNCTION public.sync_device_logs_to_attendance(date) IS
  'Matches each stored UTC biometric punch to only its exact generated routine period.';
