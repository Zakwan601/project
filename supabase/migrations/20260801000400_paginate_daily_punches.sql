/* Return one paginated row per student and punch date for the admin table. */
...
REVOKE ALL ON FUNCTION public.get_daily_punches_page(date, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_daily_punches_page(date, integer, integer) TO authenticated;

COMMENT ON FUNCTION public.get_daily_punches_page(date, integer, integer) IS
  'Returns RLS-filtered daily biometric punch groups with server-side date filtering and pagination.';
