/* Search before pagination while preserving caller RLS and student scope. */
...
$$;
REVOKE ALL ON FUNCTION public.search_daily_punches_page(date, integer, integer, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.search_daily_punches_page(date, integer, integer, text, text) TO authenticated;
